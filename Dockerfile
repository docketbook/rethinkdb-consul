...
RUN apk add --no-cache curl make gcc g++ openssl binutils-gold python linux-headers paxctl libgcc libstdc++ gnupg && \
...
RUN wget -O containerpilot.tar.gz https://github.com/joyent/containerpilot/releases/download/2.1.1/containerpilot-2.1.1.tar.gz && \
...
ENTRYPOINT [ "/usr/local/bin/containerbuddy", "rethinkdb", "--config-file", "/etc/rethinkdb/instances.d/%i.conf"]
